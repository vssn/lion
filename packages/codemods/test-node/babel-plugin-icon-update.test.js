...
const { formatHtml } = require('../src/utils/formatting.js');
...
describe('babel-plugin-icon-update', () => {
  ...
  // TODO
  it.skip('notifies about non template literal expressions', () => {
  ...
  it('works for icon extensions', () => {
    ...
    const result = executeBabel(code, { iconTagName: 'my-icon', oldToNewMap });
    expect(formatHtml(result)).to.equal(formatHtml(output));
  });
});
